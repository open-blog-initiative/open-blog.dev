...
async function downloadImages(images, options = {}) {
  ...
  for (const image of images) {
    ...
    const response = await (await axios.get(image).response).buffer()
    fs.writeFileSync(`./${file}`, response, "base64")
...
async function getPost(mediumURL) {
  ...
  return Promise.all(promises)
    .then(async results => {
      ...
      if (output) {
        if (!!images.length) {
          outputPath = `${output}/blog/slashgear/${story.slug}.md`
        }
...
const mediumURL = process.argv[0]

assert.strictEqual(
  process.argv.length,
  4,
  `Only 2 parameters allowed, found ${process.argv.length}`
)
assert.strictEqual(
  /^[a-z\d](?:[a-z\d]|-(?=[a-z\d])){0,38}$/i.test(process.argv[2]),
  true,
  `Not a valid github username`
)

getPost(process.argv[3])
